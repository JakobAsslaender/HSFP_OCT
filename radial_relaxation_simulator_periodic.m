function [y,z,dy,dz] = radial_relaxation_simulator_periodic(theta, TR, T1, T2, B0, B1)
...
for n = Nspin:-1:1
    ...
    r = TR/T1(n) .* a .* (int_cta - a(end)/(a(end)-1) * int_cta(end));
...
%     dadT1 =  a    .*       TR        .* cumsum(ctheta.^2/T1(n)^2);
%     drdT1 = TR/T1(n)  .* (dadT1 - a/T1(n)) .* (int_cta - a(end)/(a(end)-1) .* int_cta(end)) + ...
%             TR/T1(n)  .* a .* (cumsum(-dadT1./a./a .* ctheta) - ...
%                                dadT1(end)        /(a(end)-1)    .* int_cta(end) + ...
%                                dadT1(end).*a(end)/(a(end)-1).^2 .* int_cta(end) - ...
%                                   a(end)/(a(end)-1) .* sum(-dadT1./a./a .* ctheta));
...
%     drdT2 = TR/T1(n)  .* dadT2 .* (int_cta - a(end)/(a(end)-1) .* int_cta(end)) + ...
%             TR/T1(n)  .*  a    .* (cumsum(-dadT2./a./a .* ctheta) - ...
%                                dadT2(end)/(a(end)-1) .* int_cta(end) + ...
%                                   a(end)/(a(end)-1).^2 .* int_cta(end) .* dadT2(end) - ...
%                                   a(end)/(a(end)-1) .* sum(-dadT2./a./a .* ctheta));
    ...
    if nargout > 2        
        ...
        drdtheta = - (stheta./a + diag(dadtheta).'./a./a .* ctheta).' * a;
        ...
        drdtheta = drdtheta + bsxfun(@times, dadtheta, int_cta.' - a(end)/(a(end)-1) * int_cta(end));

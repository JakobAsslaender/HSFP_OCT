...
% in case some jobs failed...
% try
%     load(['~/mygs/20190718_SimulatedSignals/ijob_', num2str(ijob)])
%     exit;
% catch
% disp(['ijob ', num2str(ijob), ' not yet done, going for it...']);
% end
%%
bDerivatives = true;
bfinite_pulse_correction = true;
load('/Users/liukangning/Downloads/MRF/MRF_CRB/data/OCT_resutl_MT_100_3s_low_SAR.mat', 'x');
TR = 4.5e-3;
t = TR/2:TR:3;
% m0s = .2;
% T1 = 1;
...
%%
if uniform_sampling
    save(['/Users/liukangning/Downloads/MRF/MRF_CRB/Uniform/ijob_', num2str(ijob)], 'm0s', 'T1', 'T2f', 'R', 'T2s', 's', 'CRB_all');
else
    save(['~/mygs/20190819_SimulatedSignals/Gaussian/ijob_', num2str(ijob)], 'm0s', 'T1', 'T2f', 'R', 'T2s', 's', 'CRB_all');
end
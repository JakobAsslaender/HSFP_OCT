...
for n = Nspin:-1:1
    ...
    if nargout > 2
        ...
        for k = Npulse:-1:1
            ...
            drdtheta(k, :   ) = dadtheta .* (r0 + TR/T1(n) * sum(cos(theta(1:k))./a(1:k)));
            ...
            drdT1dtheta(k, :   ) = dadT1dtheta .* (r0 + TR/T1(n)   * sum(cos(theta(1:k))./a(1:k))) + ...
                                   dadtheta    .* (   - TR/T1(n)^2 * sum(cos(theta(1:k))./a(1:k))  - ...
                                                        TR/T1(n)   * sum(cos(theta(1:k))./a(1:k).^2.*dadT1(1:k)));
            drdT1dtheta(k,k:end) = drdT1dtheta(k,k:end) - dadT1(k:end) .* TR/T1(n)    .* (sin(theta(k))/a(k) + dadtheta(k)*cos(theta(k))/a(k)^2) + ...
                                                           ...
                                                           a   (k:end) .* TR/T1(n)    .* (sin(theta(k))/a(k).^2.*dadT1(k) - dadT1dtheta(k)*cos(theta(k))/a(k)^2 + 2 * dadtheta(k)*cos(theta(k))/a(k)^3 * dadT1(k));
            
            dadT2dtheta(  k:end) = dadT2(k:end) * TR * sin(2*theta(k)) * (1/T1(n) - 1/T2(n)) + ...
                                    a   (k:end) * TR * sin(2*theta(k)) * (1/T2(n)^2);
            drdT2dtheta(k, :   ) = dadT2dtheta .* (r0 + TR/T1(n)   * sum(cos(theta(1:k))./a(1:k))) - ...
                                   dadtheta    .*       TR/T1(n)   * sum(cos(theta(1:k))./a(1:k).^2.*dadT2(1:k));
